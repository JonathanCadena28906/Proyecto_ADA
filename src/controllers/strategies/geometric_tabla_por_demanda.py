...
class GeometricSIA(SIA):
    ...
    def _calcular_tabla_costos(self):
        """
        Inicializa la tabla de costos vacía. Los valores se calcularán bajo demanda.
        """
        # Simplemente inicializar un diccionario vacío - no precalcular nada
        self.tabla_transiciones = {}
        print(f"Tabla de costos inicializada en modo bajo demanda")

    def _obtener_costo_transicion(self, var_idx, estado_i, estado_j):
        """
        Obtiene el costo de transición. Lo calcula solo si no existe, o lo recupera de la caché.
        ...
        """
        clave = (var_idx, estado_i, estado_j)
        
        # Si el costo ya está calculado, retornarlo de la caché
        if clave in self.tabla_transiciones:
            return self.tabla_transiciones[clave]
        
        # Calcular el costo y almacenarlo para futuras consultas
        n_bits = len(self.sia_subsistema.dims_ncubos)
        costo = self._calcular_costo_transicion_recursivo(estado_i, estado_j, var_idx, n_bits)
    ...
    def _exportar_tabla_excel(self, n_vars: int, n_estados: int, n_bits: int):
        """
        Exporta la tabla de costos calculados hasta el momento a Excel con formato little-endian.
        """
        # Crear hojas separadas para cada variable
        with pd.ExcelWriter(f"tabla_costos_geometric_bajo_demanda.xlsx") as writer:
            
            for var_idx in range(n_vars):
                # Crear matriz de costos para esta variable con los valores calculados
                matriz_costos = np.zeros((n_estados, n_estados))
                
                # Rellenar solo los valores que se han calculado bajo demanda
                for i in range(n_estados):
                    for j in range(n_estados):
                        clave = (var_idx, i, j)
                        # Solo incluir los valores que ya están en la tabla
                        if clave in self.tabla_transiciones:
                            matriz_costos[i, j] = self.tabla_transiciones[clave]
                        else:
                            matriz_costos[i, j] = np.nan  # NaN para valores no calculados
            ...
            # Añadir una hoja con estadísticas de uso
            n_calculados = len(self.tabla_transiciones)
            n_total_posible = n_vars * n_estados * n_estados
            porcentaje = (n_calculados / n_total_posible) * 100
            
            df_stats = pd.DataFrame({
                'Métrica': ['Valores calculados', 'Total posible', 'Porcentaje calculado'],
                'Valor': [n_calculados, n_total_posible, f"{porcentaje:.2f}%"]
            })
    ...
    def _analizar_transiciones_estandar(self, n_vars, n_bits, estado_inicial) -> Tuple[List[dict], Optional[Tuple[Set[int], Set[int]]]]:
        ...
        MODIFICADO: Usa solo variables disponibles en el alcance para calcular el promedio.
        """
        ...
        variables_disponibles_sistema = set(range(n_bits))
        ...
        print(f"Variables disponibles en el sistema: {[chr(65 + i) for i in sorted(variables_disponibles_sistema)]}")
        print(f"Variables disponibles en el mecanismo: {[chr(65 + i) for i in sorted(variables_disponibles_mecanismo)]}")
        print(f"Variables disponibles para alcance: {[chr(65 + i) for i in sorted(variables_disponibles_alcance)]}")
        
        # PASO 2: Analizar todas las transiciones y calcular promedios
        ...
        for estado_destino in range(1, n_estados):  # t(000,001) hasta t(000,111)
            ...
            # Crear diccionario para almacenar todos los costos
            costos_dict = {}
            
            # Calcular costos para todas las variables
            for var_idx in sorted(variables_disponibles_sistema):
                if var_idx < n_bits:  # Verificar que esté dentro del rango válido
                    costo = self._obtener_costo_transicion(var_idx, estado_inicial, estado_destino)
                    var_letra = chr(65 + var_idx)
                    costos_dict[var_idx] = costo
                    #print(f"Variable {var_letra}: costo = {costo:.4f}")
            
            # CAMBIO CLAVE: Calcular promedio SOLO con variables disponibles en el alcance
            costos_alcance = []
            for var_idx in sorted(variables_disponibles_alcance):
                if var_idx in costos_dict and costos_dict[var_idx] < float('inf'):
                    costos_alcance.append(costos_dict[var_idx])
            
            # Calcular promedio solo con variables de alcance
            if costos_alcance:
                promedio_alcance = sum(costos_alcance) / len(costos_alcance)
                print(f"Promedio de costos (solo variables de alcance): {promedio_alcance:.4f}")
                
                # Convertir diccionario a lista para mantener compatibilidad
                costos_lista = [costos_dict.get(i, float('inf')) for i in range(n_vars)]
                
                transiciones_con_promedio.append({
                    'estado_destino': estado_destino,
                    'estado_destino_bin': estado_destino_bin,
                    'costos_variables': costos_lista,  # Mantener formato de lista para compatibilidad
                    'promedio': promedio_alcance      # Promedio solo de variables de alcance
                })
            else:
                print("Transición descartada por costos infinitos en variables de alcance")
    
        # PASO 3: Seleccionar transiciones con mejor promedio (menor costo)
        ...
        # Calcular el promedio general solo con variables de alcance
        promedios = [trans['promedio'] for trans in transiciones_con_promedio]
        promedio_general = sum(promedios) / len(promedios)
        print(f"\nPromedio general de todas las transiciones (solo variables de alcance): {promedio_general:.4f}")
        ...
        # Mostrar información detallada de cada mejor transición
        print(f"\n=== INFORMACIÓN DETALLADA DE LAS {len(mejores_transiciones)} MEJORES TRANSICIONES ===")
        for i, trans in enumerate(mejores_transiciones):
            estado_bin = trans['estado_destino_bin']
            promedio = trans['promedio']
            costos = trans['costos_variables']
            
            # Mostrar TODAS las variables con sus costos, marcando las del alcance
            alcance_vars = []
            otras_vars = []
            
            for j in range(len(costos)):
                var_letra = chr(65 + j)
                if costos[j] < float('inf'):
                    if j in variables_disponibles_alcance:
                        alcance_vars.append(f"{var_letra}={costos[j]:.4f}*")  # Marcar variables de alcance
                    else:
                        otras_vars.append(f"{var_letra}={costos[j]:.4f}")
            
            # Unir todas las variables en una cadena, destacando primero las de alcance
            costos_str = ", ".join(alcance_vars + otras_vars)
            
            print(f"Transición {i+1}: t(000, {estado_bin}) - Promedio (solo alcance): {promedio:.4f}")
            print(f"  Costos: [{costos_str}]")
            print(f"  * Variables usadas para calcular el promedio")
        ...
        for trans in mejores_transiciones:
            ...
            print(f"\n=== ANALIZANDO TRANSACCIÓN #{mejores_transiciones.index(trans)+1} ===")
            print(f"Estado original: t(000, {trans['estado_destino_bin']})")
            ...
            costos_complemento = []
            for var_idx in range(n_vars):
                costo = self._obtener_costo_transicion(var_idx, estado_inicial, estado_complemento)
            ...
            # PASO 3.5: Permitir conjuntos vacíos (ELIMINAMOS AJUSTES FORZADOS)
            if not variables_presente and variables_futuro:
                print("NOTA: Conjunto PRESENTE vacío, se evaluará sin forzar cambios")
                # Evaluar explícitamente la partición con presente vacío
                
            if not variables_futuro and variables_presente:
                print("NOTA: Conjunto FUTURO vacío, se evaluará sin forzar cambios")
                # Evaluar explícitamente la partición con futuro vacío

            # PASO 3.6: Formar la partición (sin ajustes forzados)
            conjunto_presente = set(variables_presente)
            conjunto_futuro = set(variables_futuro)

            # Incorporar bits que se mantienen al conjunto presente
            for bit_pos in bits_que_se_mantienen:
                if bit_pos < n_bits:  # Verificar que esté dentro del rango válido
                    conjunto_presente.add(bit_pos)

            print(f"Partición resultante sin ajustes:")
            print(f"  Presente (mecanismo): {[chr(65 + i) for i in sorted(conjunto_presente)]}")
    ...
    def _formatear_resultado(self, biparticion_optima: Tuple[Set[int], Set[int]]) -> Solution:
        ...
        try:
            ...
            # CORRECCIÓN: Usar directamente los índices reales, no un rango
            variables_disponibles_sistema = set(self.sia_subsistema.indices_ncubos)
            print(f"Variables disponibles en el sistema: {[chr(65 + i) for i in sorted(variables_disponibles_sistema)]}")
            
            # Variables disponibles para mecanismo (dimensiones de los ncubos)
            variables_disponibles_mecanismo = set(self.sia_subsistema.dims_ncubos)
            
            # Variables disponibles para alcance (índices de los ncubos)
            variables_disponibles_alcance = set(self.sia_subsistema.indices_ncubos)
            ...
            # Validar conjuntos con sus respectivas variables disponibles
            conjunto_presente_valido = conjunto_presente.intersection(variables_disponibles_mecanismo)
            conjunto_futuro_valido = conjunto_futuro.intersection(variables_disponibles_alcance)
            
            if conjunto_presente != conjunto_presente_valido or conjunto_futuro != conjunto_futuro_valido:
                print("ADVERTENCIA: Algunos índices en los conjuntos no corresponden a variables disponibles para su rol")
                conjunto_presente = conjunto_presente_valido
                conjunto_futuro = conjunto_futuro_valido
                print(f"Conjuntos corregidos - Presente: {[chr(65+i) for i in conjunto_presente]}, Futuro: {[chr(65+i) for i in conjunto_futuro]}")
            
            # Calcular complementos de los conjuntos SOLO con variables disponibles para cada rol
            complemento_presente = variables_disponibles_mecanismo - conjunto_presente
            complemento_futuro = variables_disponibles_alcance - conjunto_futuro
            
            # Organizar nodos para el formato visual
            nodos_mecanismo = []
            nodos_alcance = []
            
            # PRIMER PARÉNTESIS
            # Arriba (alcance): Variables EFECTO del conjunto futuro (solo las disponibles en alcance)
            futuro_disponible = conjunto_futuro  # Ya validado arriba
            ...
            presente_disponible = conjunto_presente  # Ya validado arriba
    ...
    def _analizar_caso_especial_todos_unos(self, n_vars, n_bits, estado_inicial):
        ...
        OPTIMIZADO: Evalúa solo las variables disponibles en el alcance que cumplen
        la condición de costo <= promedio.
        """
        particiones_candidatas = []
        ...
        # PASO 3: Aplicar criterio de selección
        if costos_directos:
            promedio_costos_directos = costos_totales / len(costos_directos)
            print(f"Promedio de costos directos: {promedio_costos_directos:.4f}")
            
            # Filtrar variables con costo <= promedio
            variables_seleccionadas = [(idx, costo, letra) for idx, costo, letra in costos_directos 
                                     if costo <= promedio_costos_directos]
            
            print(f"Evaluando {len(variables_seleccionadas)}/{len(costos_directos)} variables disponibles con costo <= promedio")
        else:
            print("No se encontraron variables disponibles con costos válidos")
            return particiones_candidatas, None
        
        # PASO 4: Ordenar y evaluar las variables seleccionadas
        variables_seleccionadas.sort(key=lambda x: x[1])  # Ordenar por costo ascendente
